...
const ProductItem = ({ item }: { item: Product }) => {
  ...
  return (
    <div className="group border border-gray-3 rounded-md">
      <div className="relative overflow-hidden flex  items-center justify-center rounded-lg bg-[#F6F7FB] min-h-[270px] mb-4">
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          ...
          <button
            ...
            className="inline-flex font-medium text-custom-sm py-[7px] px-5 rounded-[5px] bg-[#E53E3E] text-white ease-out duration-200 hover:bg-[#f84444]"
          >
            Add to cart
          </button>
      ...
      <h3
        className="font-medium text-dark ml-4 ease-out duration-200 hover:text-blue mb-1.5"
        onClick={() => handleProductDetails()}
      >
        <Link href="/shop-details"> {item.title} </Link>
      </h3>

      <span className="flex items-center gap-2 font-medium text-lg ml-4">
