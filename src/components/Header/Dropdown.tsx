...
const Dropdown = ({ menuItem, stickyMenu }) => {
  ...
  return (
    ...
    >
      <a
        href="#"
        className={`hover:text-[#E53E3E] uppercase text-custom-xs font-medium text-white flex items-center gap-1.5  ${
          stickyMenu ? "xl:py-4" : "xl:py-6"
        } ${pathUrl.includes(menuItem.title) && "!text-[#E53E3E]"}`}
      >
        {menuItem.title}
        <svg
      ...
      {/* <!-- Dropdown Start --> */}
      <ul
        className={`dropdown z-99 ${dropdownToggler && "flex"} ${
          stickyMenu
      ...
      >
        {menuItem.submenu.map((item, i) => (
          <li key={i}>
            <Link
              href={item.path}
              className={`flex text-custom-sm hover:text-[#E53E3E] hover:bg-gray-1 py-[7px] px-4.5 ${
                pathUrl === item.path && "text-[#E53E3E] bg-gray-1"
              } `}
            >
              {item.title}
            </Link>
